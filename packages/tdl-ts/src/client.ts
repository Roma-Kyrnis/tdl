...
import { deepRenameKey, mergeDeepRight } from './util.ts';
import { createLogger } from './utils/logger.ts';
import { Version } from './version.ts';

// NOTE: if needed, this client can be abstracted into a different package later

const debug = createLogger('tdl:client');
const debugEmitter = createLogger('tdl:client:emitter');
const debugReceive = createLogger('tdl:client:receive');
const debugReq = createLogger('tdl:client:request');
...
export type TdjsonTdlTdlibAddon = {
  ...
  create(): TdjsonClient;
  destroy(client: TdjsonClient): void;
  execute(client: null | TdjsonClient, request: any): any | null;
  receive(client: TdjsonClient, timeout: number): Promise<any | null>;
  send(client: TdjsonClient, request: any): void;
  setLogFatalErrorCallback(fn: null | ((errorMessage: string) => void)): void;
};
...
type TdjsonCompat = {
  ...
  destroy(client: TdjsonClient): void;
  execute(client: null | TdjsonClient, request: any): any | null;
  receive(client: TdjsonClient): Promise<any | null>;
  send(client: TdjsonClient, request: any): void;
  setLogFatalErrorCallback(fn: null | ((errorMessage: string) => void)): void;
  setLogMessageCallback(
    maxVerbosityLevel: number,
...
function tdjsonCompat(td: TdjsonTdlTdlibAddon | Tdjson): TdjsonCompat {
  const foundTdlTdlibAddon = (td as any).getName?.() === 'addon';
  if (!foundTdlTdlibAddon) {
    const tdjson: Tdjson = td as any;
    return {
      ...tdjson,
      execute: (client, request): any => {
        const response = tdjson.execute(client, JSON.stringify(request));
        if (response === null) return null;
        return JSON.parse(response);
      },
      send: (client, request): void => {
        tdjson.send(client, JSON.stringify(request));
      },
      receive: async (client): Promise<any> => {
  ...
  const tdjsonOld: TdjsonTdlTdlibAddon = td as any;
  ...
  return {
    ...
    async receive(client): Promise<any> {
    ...
    setLogMessageCallback: () => unavailable('setLogMessageCallback'),
    tdn: {
      init: () => unavailable('tdn.init'),
      createClientId: () => unavailable('tdn.createClientId') as any,
      send: () => unavailable('tdn.send') as any,
      receive: () => unavailable('tdn.receive') as any,
      execute: () => unavailable('tdn.execute') as any,
    },
  };
}

export type TDLibParameters = Omit<Td.tdlibParameters$Input, '_'>;
// export type TDLibParameters = Omit<Td$setTdlibParameters, '_'>

export type LoginUser = {
  type: 'user';
  /** Rewrote this functions to use only for notify user that he/she needs to send specific data to TelegramBot */
  onPhoneNumber: (retry?: boolean) => Promise<void>;
  /** Not available now: */
  // onEmailAddress: () => Promise<void>;
  // onEmailCode: () => Promise<void>;
  /** Not available now. */
  confirmOnAnotherDevice: (link: string) => void;
  onAuthCode: (retry?: boolean) => Promise<void>;
  onPassword: (passwordHint: string, retry?: boolean) => Promise<void>;
  onName: () => Promise<void>;
  onStateReady: () => Promise<void>;
...
//   type: 'user',
//   onPhoneNumber: prompt.getPhoneNumber,
//   onEmailAddress: prompt.getEmailAddress,
//   onEmailCode: prompt.getEmailCode,
//   confirmOnAnotherDevice: prompt.confirmOnAnotherDevice,
//   onAuthCode: prompt.getAuthCode,
//   onPassword: prompt.getPassword,
//   onName: prompt.getName,
// };
...
export type On = ((event: 'update', listener: (update: Td$Update) => void) => Client) &
...
export type Emit = ((event: 'update', update: Td$Update) => void) &
...
export type Off = ((event: 'update', listener: (...args: any[]) => any, once?: boolean) => void) &
...
// const emptyDetails = (): Readonly<Record<any, any>> => Object.freeze({});

const invariant = (cond: boolean, msg: string = 'Invariant violation'): void => {
...
export class Client {
  ...
  constructor(
    ...
    this._options = mergeDeepRight(defaultOptions, options) as StrictClientOptions;
    ...
    if (this._options.verbosityLevel != null && this._options.verbosityLevel !== 'default') {
      ...
      this.execute({
        ...
        new_verbosity_level:
          typeof this._options.verbosityLevel === 'number'
            ? this._options.verbosityLevel
            : parseInt(this._options.verbosityLevel),
      });
    ...
    if (this._options.bare) this._initialized = true;
  ...
  _catchError(err: Td$error | TdlError): void {
    ...
    if (this._loginDefer.isPending()) this._loginDefer.reject(err);
    else this.emit('error', err);
  }

  // login: (getLoginDetails?: () => LoginDetails) => Promise<void> = (
  login: (getLoginDetails: () => StrictLoginDetails) => Promise<void> = getLoginDetails => {
    debug('client.login()');
    this._emitter.once('auth-needed', () => {
      this._loginDetails = getLoginDetails();
      // this._loginDetails = mergeDeepRight(defaultLoginDetails, getLoginDetails());
  ...
  /** @deprecated */
  connectAndLogin: (fn: () => StrictLoginDetails) => Promise<void> = fn => {
  ...
  invoke: Invoke = async request => {
    ...
    this._requestId++;
    if (this._requestId >= Number.MAX_SAFE_INTEGER) this._requestId = 0;
    // $FlowIgnore[prop-missing]
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-ignore
    request['@extra'] = id;
    const receiveResponse = new Promise<any>((resolve, reject) => {
  ...
  setLogFatalErrorCallback = (fn: null | ((msg: string) => void)): void => {
  ...
  _send(request: { _: string; [k: any]: any }): void {
    ...
    if (this._tdn) this._tdlib.tdn.send(this._clientId, JSON.stringify(tdRequest));
    else this._tdlib.send(this._client, tdRequest);
  }

  _sendTdl(request: Record<any, any>): void {
  ...
  _handleUpdate(update: Td$Update): void {
    ...
    switch (update._) {
      ...
      default:
        if (this._options.skipOldUpdates && this._connectionState._ === 'connectionStateUpdating') {
          return;
        }
        this.emit('update', update);
    }
  ...
  async _handleAuth(update: Td$updateAuthorizationState): Promise<void> {
    ...
    switch (authorizationState._) {
      case 'authorizationStateWaitTdlibParameters':
        if (this._version.gte(TDLIB_1_8_6)) {
          if ((this._options.tdlibParameters as any)._ != null)
    ...
    console.log(authorizationState._);

    switch (authorizationState._) {
      case 'authorizationStateReady': {
        const loginDetails = this._needUserLogin();
        this._loginDefer.resolve();
        return loginDetails.onStateReady();
      }
      ...
      case 'authorizationStateWaitPhoneNumber': {
        ...
        switch (loginDetails.type) {
          case 'user':
            return loginDetails.onPhoneNumber(false);
          // return this._sendTdl({
      ...
      case 'authorizationStateWaitEmailAddress': {
        debug('not supported authorizationStateWaitEmailAddress');
        return;
        // return this._sendTdl({
        //   _: 'setAuthenticationEmailAddress',
        //   email_address: await loginDetails.onEmailAddress(),
        // });
      }
      ...
      case 'authorizationStateWaitEmailCode': {
        debug('not supported authorizationStateWaitEmailCode');
        return;
        // return this._sendTdl({
      ...
      case 'authorizationStateWaitPassword': {
        ...
        loginDetails.onPassword(passwordHint, false);
        // const loginDetails = this._needUserLogin();
        // const passwordHint = authorizationState.password_hint;
  ...
  async _handleUserError(error: Td$error, loginDetails: LoginUser): Promise<void> {
    switch (error.message) {
      ...
      case 'PHONE_CODE_INVALID':
        return loginDetails.onAuthCode(true);
      // return this._sendTdl({
      //   _: 'checkAuthenticationCode',
      ...
      case 'PHONE_NUMBER_INVALID':
        return loginDetails.onPhoneNumber(true);
      // return this._sendTdl({
      //   _: 'setAuthenticationPhoneNumber',
      ...
      case 'PASSWORD_HASH_INVALID':
        return loginDetails.onPassword('', true);
      // return this._sendTdl({
      //   _: 'checkAuthenticationPassword',
