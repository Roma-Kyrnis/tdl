...
export const getPhoneNumber = (retry?: boolean): Promise<string> =>
  prompt(retry ? 'Invalid phone number, please re-enter: ' : 'Please enter your phone number: ');

export const getEmailAddress = (): Promise<string> => prompt('Please enter your email address: ');
...
export const getAuthCode = (retry?: boolean): Promise<string> =>
  prompt(
    retry ? 'Wrong auth code, please re-enter: ' : 'Please enter the auth code you received: ',
  );
...
export const getName: GetName = async () => ({
  ...
  lastName: await prompt('Please enter your last name (optional): '),
});
